...
pub fn applyPatch(
    ...
    var parent_to_removed_child: std.AutoArrayHashMapUnmanaged(hash.HashInt(repo_opts.hash), hash.HashInt(repo_opts.hash)) = .empty;
    defer parent_to_removed_child.deinit(allocator);
    var parent_to_added_child: std.AutoArrayHashMapUnmanaged(hash.HashInt(repo_opts.hash), [LineId(repo_opts.hash).byte_size]u8) = .empty;
    defer parent_to_added_child.deinit(allocator);
        ...
        switch (std.enums.fromInt(ChangeKind, change_kind) orelse return error.InvalidEnumTag) {
            .new_edge => {
                ...
                // add to parent's children
                {
                    ...
                    try parent_to_added_child.put(allocator, parent_line_id_hash, line_id_bytes);
            ...
            .delete_line => {
                ...
                if (try child_to_parent.getCursor(line_id_hash)) |parent_cursor| {
                    ...
                    try parent_to_removed_child.put(allocator, parent_line_id_hash, line_id_hash);
...
fn writePatch(
    ...
    var patch_entries: std.ArrayList([]const u8) = .empty;
    defer patch_entries.deinit(allocator);

    var patch_offsets: std.ArrayList(u64) = .empty;
