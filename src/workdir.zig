        ...
        fn addEntries(
            ...
            switch (meta.kind) {
                ...
                .directory => {
                    ...
                    var child_untracked: std.ArrayList(Status(repo_kind, repo_opts).Entry) = .empty;
...
pub fn migrate(
    ...
    for (tree_diff.changes.keys(), tree_diff.changes.values()) |path, change| {
        ...
        if (switch_result_maybe) |switch_result| {
            ...
            if (compareTreeToIndex(repo_kind, repo_opts, change.old, entry_maybe) != .none and compareTreeToIndex(repo_kind, repo_opts, change.new, entry_maybe) != .none) {
                ...
                try switch_result.result.conflict.stale_files.put(switch_result.arena.allocator(), path, {});
            } else {
                const meta = fs.Metadata.init(io, state.core.work_dir, path) catch |err| switch (err) {
                    error.FileNotFound, error.NotDir => {
                        ...
                        if (untrackedParent(repo_kind, repo_opts, io, state.core.work_dir, path, index)) |_| {
                            ...
                            if (entry_maybe) |_| {
                                ...
                                try switch_result.result.conflict.untracked_overwritten.put(switch_result.arena.allocator(), path, {});
                            } else {
                                try switch_result.result.conflict.untracked_removed.put(switch_result.arena.allocator(), path, {});
                            }
                        }
                        continue;
                    },
                    ...
                    .directory => {
                        ...
                        if (try untrackedFile(repo_kind, repo_opts, io, allocator, state.core.work_dir, path, index)) {
                            ...
                            if (entry_maybe) |_| {
                                ...
                                try switch_result.result.conflict.stale_dirs.put(switch_result.arena.allocator(), path, {});
                            }
                        }
                    },
                    else => {},
    ...
    for (index.entries.keys(), index.entries.values()) |path, *index_entries_for_path| {
        ...
        // and the work dir is (optionally) updated
            ...
            try add_files.put(allocator, path, .{ .oid = conflict_entry.oid, .mode = conflict_entry.mode });
        }
    }
            ...
            if (.conflict != self.result) {
                self.result = .{
                    .conflict = .{
                        .stale_files = .empty,
                        .stale_dirs = .empty,
                        .untracked_overwritten = .empty,
                        .untracked_removed = .empty,
                    },
                };
            }
        }
